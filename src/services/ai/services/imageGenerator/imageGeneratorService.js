...
import {
  generateQwenImage,
  getQwenSupportedSizes,
  getQwenSupportedStyles,
} from "../../providers/qwen.js";
import {
  generateStabilityImage,
  getStabilitySupportedSizes,
  getStabilitySupportedStyles,
} from "../../providers/stabilityAI.js";
import {
...
// Options: pollinations (no API key), qwen, stability, huggingface, openai
// Priority: pollinations > qwen > stability > huggingface > openai
const getDefaultProvider = () => {
  ...
  if (process.env.STABILITY_API_KEY || process.env.STABLE_DIFFUSION_API_KEY) {
    return "stability";
  }
...
export class AIImageGeneratorService {
  ...
  // Supports: Pollinations (free, no API key), Qwen, Stability AI, Hugging Face, DALL·E 3
  async generateImage(prompt, options = {}) {
    ...
    switch (activeProvider) {
      ...
      case "qwen":
        return await this.generateImageWithQwen(prompt, options);
      case "stability":
  ...
  // Generate image using Qwen (DashScope) - Free tier available
  async generateImageWithQwen(prompt, options = {}) {
    const { saveToStorage = true, userId = null } = options;
    const {
      size = "1024x1024",
      quality = "standard", // Not used for Qwen, but kept for compatibility
      style = "vivid",
      n = 1,
    } = options;

    // Map size and style to Qwen format
    const qwenSize = QWEN_SIZE_MAP[size] || "1024*1024";
    const qwenStyle = QWEN_STYLE_MAP[style.toLowerCase()] || "<auto>";
    ...
    try {
      console.log("🎨 Generating image with Qwen (DashScope - Free tier)...");

      const startTime = Date.now();

      // Generate with Qwen
      const qwenResult = await generateQwenImage(enhancedPrompt, {
        size: qwenSize,
        style: qwenStyle,
        n: Math.min(n, 4), // Qwen supports up to 4 images
      });

      const duration = Date.now() - startTime;
      const dalleImageUrl = qwenResult.imageUrl; // Primary image URL
      ...
      if (saveToStorage && userId) {
        try {
          console.log("💾 Saving Qwen image to permanent storage...");
          const storageResult = await downloadAndSaveImage(
            dalleImageUrl,
            userId
          );
          ...
          console.log(
            "✅ Qwen image saved to permanent storage:",
            permanentImageUrl
          );
        } catch (storageError) {
          console.error(
            "⚠️ Failed to save Qwen image to storage, using original URL:",
            storageError.message
          );
        }
      }
      ...
      throw new Error(
        `Qwen image generation failed: ${error.message || "Unknown error"}`
      );
    }
  }
  ...
  async generateImageWithStability(prompt, options = {}) {
    ...
    // Map style to Stability AI style preset
    const stabilityStyle = STABILITY_STYLE_MAP[style.toLowerCase()] || null;
    ...
    try {
      ...
      const stabilityResult = await generateStabilityImage(enhancedPrompt, {
        size: size,
        style: stabilityStyle,
      });

      const duration = Date.now() - startTime;
      let imageUrl = stabilityResult.imageUrl; // Base64 data URL
      ...
      if (saveToStorage && userId && imageUrl.startsWith("data:image")) {
        try {
          console.log("💾 Saving Stability AI image to permanent storage...");
          const storageResult = await saveBase64Image(
            imageUrl,
            userId,
            "stability"
          );
          ...
          console.log("✅ Stability AI image saved:", permanentImageUrl);
        } catch (storageError) {
          console.error(
            "⚠️ Failed to save Stability AI image:",
            storageError.message
          );
        }
      }
  ...
  getSupportedSizes() {
    switch (this.provider) {
      ...
      case "qwen":
        return ["1024x1024", "1280x720", "720x1280"];
      case "stability":
        return getStabilitySupportedSizes();
      case "huggingface":
      case "hf":
